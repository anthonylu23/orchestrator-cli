...
2. Pulls `job.image`.
3. Runs `docker run --rm --gpus all`.
4. Mounts `/tmp/switchboard` into the container.
5. Passes Switchboard runtime environment variables.
6. Writes container stdout/stderr to `/tmp/switchboard/logs.txt`.
...
2. No private registry credential helper yet.
