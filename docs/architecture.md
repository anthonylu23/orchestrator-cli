...
Existing installs using `~/.orchestrator-cli` remain readable. Home resolution prefers `--home`, `SWITCHBOARD_CLI_HOME`, deprecated `ORCHESTRATOR_CLI_HOME`, an existing legacy home, then the new default `~/.switchboard-cli`. Within a selected home, the state layer opens `switchboard.db` and falls back to `orchestrator.db` only when the new DB file is absent and the legacy DB exists.
...
During the rename compatibility window, runtime metadata is injected under both `SWITCHBOARD_*` and deprecated `ORCHESTRATOR_*` variable names. New providers and examples should use the `SWITCHBOARD_*` names.
