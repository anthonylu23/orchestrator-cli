...
18. `internal/staging`: provider-independent object-storage runtime env construction for shared checkpoint/data prefixes plus managed GCS/S3 upload for bundled data inputs.
19. `runtime/container`: optional container-side helpers, including URI data materialization before the training command starts.
...
3. Generalize provider-specific resource status reporting before adding resource adoption for resources created outside the current Switchboard run.
...
6. Add provider-native registry secret handling where cloud APIs expose a safer path than launch user data.
7. Add the next image-first provider after validating its create, status, log, cancel, pricing, and cleanup API shape against the provider contract.
