...
Planned commands not implemented yet include explicit `resume`. Planned provider work now focuses on managed object-store upload/download staging, provider-native registry secret handling, additional cloud adapters, and richer provider-specific checkpoint integrations.
...
Shared object-storage staging env vars are configured under `staging`. Switchboard currently supports `s3://` and `gs://` prefixes, injects per-run checkpoint/data prefixes, and exposes declared URI inputs as name-scoped env vars for the container. See [Object Storage Staging](docs/object-storage-staging.md).
...
9. Later provider work includes Hyperbolic, RunPod, China cloud live verification/promotion, automatic object-store upload/download staging, fan-out sweeps, richer terminal UI, and optional hosted control plane.
